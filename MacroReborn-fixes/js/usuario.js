...
if (!usuario) {
  ...
  // persona estaba realmente conectada. Se calcula igual que en
  // Comunidad (js/comunidad.js -> estaConectado): en línea si tuvo
  // actividad (last_login) en los últimos MINUTOS_CONECTADO minutos.
  const estaConectadoEstePerfil = (() => {
    if (!usuario.last_login) return false;
    const ultima = new Date(usuario.last_login).getTime();
    if (isNaN(ultima)) return false;
    const minutos = (typeof MINUTOS_CONECTADO !== "undefined") ? MINUTOS_CONECTADO : 5;
    return (Date.now() - ultima) <= minutos * 60 * 1000;
  })();

  document.getElementById("estado").textContent = estaConectadoEstePerfil
    ? "🟢 En línea"
    : "⚪ Desconectado";
  ...
  // last_login real que ya viaja en "usuario".
