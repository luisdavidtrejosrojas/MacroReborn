...
if(nav){
    ...
    // Hub global del portal. NO usa .sesion-extra porque ese selector se
    // limpia a continuación al reconstruir los controles de sesión.
    // De esta forma los accesos permanecen visibles en todas las páginas
    // que cargan navbar.js.
    if (!nav.querySelector(".mr-portal-hub")) {
    ...
    // Estilos del hub: se inyectan una sola vez para no obligar a modificar
    // todos los CSS de cada página.
    if (!document.getElementById("mrPortalHubStyles")) {
        const estiloHub = document.createElement("style");
        estiloHub.id = "mrPortalHubStyles";
        estiloHub.textContent = `
            .mr-portal-hub{position:relative;display:inline-flex;align-items:center;margin-right:8px;z-index:210;}
            .mr-portal-hub-btn{border:0;background:transparent;color:inherit;font:inherit;font-weight:700;cursor:pointer;padding:6px 8px;border-radius:8px;white-space:nowrap;}
            .mr-portal-hub-btn:hover,.mr-portal-hub-btn[aria-expanded="true"]{background:rgba(148,163,184,.12);}
            .mr-portal-hub-menu{position:absolute;top:calc(100% + 8px);left:0;min-width:190px;padding:7px;background:#111c33;border:1px solid rgba(148,163,184,.18);border-radius:12px;box-shadow:0 16px 40px rgba(0,0,0,.35);opacity:0;transform:translateY(-5px);pointer-events:none;transition:opacity .15s ease,transform .15s ease;}
            .mr-portal-hub-menu.abierto{opacity:1;transform:translateY(0);pointer-events:auto;}
            .mr-portal-hub-menu a{display:block;color:#e2e8f0;text-decoration:none;padding:9px 10px;border-radius:8px;font-size:13px;font-weight:600;}
            .mr-portal-hub-menu a:hover{background:rgba(99,102,241,.15);color:#fff;}
            @media(max-width:760px){.mr-portal-hub{margin-right:2px}.mr-portal-hub-btn{font-size:12px;padding:6px}.mr-portal-hub-menu{right:0;left:auto;min-width:180px}}
        `;
        document.head.appendChild(estiloHub);
    }

    const mrPortalHubBtn = document.getElementById("mrPortalHubBtn");
    const mrPortalHubMenu = document.getElementById("mrPortalHubMenu");
    const mrPortalHub = document.getElementById("mrPortalHub");

    if (mrPortalHubBtn && mrPortalHubMenu && !mrPortalHubBtn.dataset.bound) {
        mrPortalHubBtn.dataset.bound = "1";
        const cerrarPortalHub = () => {
            mrPortalHubMenu.classList.remove("abierto");
            mrPortalHubBtn.setAttribute("aria-expanded", "false");
        };
        mrPortalHubBtn.addEventListener("click", (e) => {
            e.stopPropagation();
            const abierto = mrPortalHubMenu.classList.toggle("abierto");
            mrPortalHubBtn.setAttribute("aria-expanded", abierto ? "true" : "false");
        });
        document.addEventListener("click", (e) => {
            if (!mrPortalHub || mrPortalHub.contains(e.target)) return;
            cerrarPortalHub();
        });
        document.addEventListener("keydown", (e) => {
            if (e.key === "Escape") cerrarPortalHub();
        });
    }
